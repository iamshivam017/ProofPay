...
const requestSchemaPath = new URL("../src/lib/api/request-schema.ts", import.meta.url);
...
test("the public verification route preserves the policy execution gate", async () => {
  const [route, requestSchema] = await Promise.all([
    readFile(routePath, "utf8"),
  ...
  assert.match(requestSchema, /MAX_REQUEST_AMOUNT_WEI = parseEther\("0\.001"\)/);
});
